...
//
// From that grid: v increases toward the FINGERTIPS (v≈0 is the knuckle line, v≈−0.29 the wrist
// crease, v≈+0.40 past the fingertips) and the THUMB is at NEGATIVE u.
enum HandSheet {
    static let wrist    = SIMD2<Float>( 0.010, -0.290)   // wrist-crease centre
    static let knuckles = SIMD2<Float>( 0.170,  0.000)   // MCP-line centre
    static let thumbRef = SIMD2<Float>(-0.350,  0.130)   // out along the abducted thumb

    /// Unit vectors of the sheet's hand frame.
    static var distal: SIMD2<Float> { simd_normalize(knuckles - wrist) }
    static var radial: SIMD2<Float> {
        let p = SIMD2<Float>(-distal.y, distal.x)
        return simd_dot(p, thumbRef - wrist) > 0 ? p : -p
    }
    static var palmLength: Float { simd_length(knuckles - wrist) }
    ...
    /// at which all twelve hand points still land DIRECTLY on the silhouette (no spiral snap). 0.90
    ...
    static let acrossScale: Float = 0.90

    static func uv(along: Float, across: Float) -> SIMD2<Float> {
        wrist + distal * (along * palmLength) + radial * (across * acrossScale * palmLength)
    }
